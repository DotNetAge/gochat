...
func TestClient_Chat_Full(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		resp := openaicompat.ChatCompletionResponse{
			...
			Usage: openaicompat.Usage{TotalTokens: 100},
		}
...
func TestClient_Options(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		resp := openaicompat.ChatCompletionResponse{
			Choices: []openaicompat.Choice{{Message: openaicompat.Message{Content: "ok"}}},
		}
		json.NewEncoder(w).Encode(resp)
	}))
	defer server.Close()
...
func TestNew_Validation(t *testing.T) {
	_, err := New(Config{Config: base.Config{Model: "deploy"}, Endpoint: "http://test"})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "api key")

	_, err = New(Config{Config: base.Config{APIKey: "key"}, Endpoint: "http://test"})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "model")

	_, err = New(Config{Config: base.Config{APIKey: "key", Model: "deploy"}})
	assert.Error(t, err)
