...
type ProgressCallback func(modelName, fileName string, downloaded, total int64)
...
func (d *Downloader) getModelInfoByName(modelName string) (ModelInfo, error) {
	for _, info := range d.GetModelInfo() {
		if info.Name == modelName {
			return info, nil
		}
	}
	return ModelInfo{}, fmt.Errorf("model not found: %s", modelName)
}
